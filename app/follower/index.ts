...
program
  ...
  .description(
    'Listens for events from the blockchain, saves them to file,' + ' and posts them to the API',
  )
...
program
  .command('push')
  .description(
    'Pushes data in the data directory to the API.' + ' Can push all data or a specific file.',
  )
  .option('-u, --url [url]', 'URL of the api to post request data to', 'http://localhost:8000')
  .option(
