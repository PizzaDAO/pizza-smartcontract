...
let state: AppContext = {
  app: {
    ...
    unredeemedBoxCount: () =>
      state.app.countTotal.existingBoxes - state.app.countTotal.existingPizzas,
  },
  contract: undefined,
  ...
  user: {
    ...
    pizzaTokens: [],
    unredeemedBoxTokens: () =>
      state.user.boxTokens.filter((box) => {
...
const DOM = {
  ...
  refreshState: (context: AppContext) => {
    ...
    // redeem button
    if (
      context.app.pizzaSaleIsActive &&
      context.wallet.address &&
      context.wallet.contract &&
      unclaimedBoxes > 0
    ) {
...
const actions = {
  ...
  app: {
    ...
    redeemBox: async (boxTokenId: number, desiredRecipe: number) => {
      ...
      await state.wallet.contract?.redeemRarePizzasBox(boxTokenId, desiredRecipe, {
        gasLimit: 300000,
      })
      await actions.contract.refreshContract()
    },
  ...
  contract: {
    ...
    refreshSupply: async () => {
      ...
      console.log(
        `maxSupply: ${maxSupply} boxTotalSupply: ${boxTotalSupply} pizzaTotalSupply: ${pizzaTotalSupply}`,
      )
  ...
  user: {
    ...
    refreshUserBalance: async () => {
      ...
      const boxTokenBalance: BigNumber = await state.wallet.boxContract?.balanceOf(
        state.wallet.address,
      )
      const pizzaTokenBalance: BigNumber = await state.wallet.contract?.balanceOf(
        state.wallet.address,
      )
      ...
      if (
        state.user.balance.boxTokenCount !== boxTokenBalance.toNumber() ||
        state.user.balance.pizzaTokenCount !== pizzaTokenBalance.toNumber()
      ) {
