...
const elements = users.map((x) =>
    ethers.utils.solidityKeccak256(["address"], [x.address])
);
const elements2 = users.map((x) =>
    ethers.utils.solidityKeccak256(['address', 'uint256'], [x.address, 2])
);
const tree = new MerkleTree(elements, keccak256, { sort: true });
const tree2 = new MerkleTree(elements2, keccak256, { sort: true });
export const merkleTree = { tree: tree, tree2: tree2, root: tree.getHexRoot(), root2: tree2.getHexRoot(), elements: elements, elements2: elements2 }
