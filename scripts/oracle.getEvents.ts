...
// get oracle requests that did not complete
async function main() {
  ...
  const completedRequestIds = new Set(
    apiEvents.map((i) => i.args && i.args['requestId']),
  )
  console.log(completedRequestIds)

  const incompleteEvents = oracleEvents.filter(
    (i) => i.args && !completedRequestIds.has(i.args['requestId']),
  )
